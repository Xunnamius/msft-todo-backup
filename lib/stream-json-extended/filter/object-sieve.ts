 ...
 * (delayed) until the entire object has been seen, after which the buffered
 * tokens will either be released downstream or discarded. This means, _in the
 * worst case_, the entire contents of each individual object will exist in
 * memory simultaneously (same as using `StreamValues`). This can be avoided
 * entirely by ensuring the entries required by `filter` the very first entries
 * in the object, or consider passing memory-efficient object subsets.
 */
