...
describe('::injectEntry', () => {
  ...
  it('writes all chunks into Duplex value token stream, then reads from it later', async () => {
    ...
    const targetObjectsTokenNames = targetObjectsTokenization.map((token) => token.name);
    ...
    for (let count = 0; count < 5; count++) {
      expectedArray.splice(
        expectedArray.findIndex(
          (token) => token.name === 'stringValue' && token.value === `object-${count + 1}`
        ) + 1,
        0,
        ...(await tokenizeObject(
          {
            ...
            valueTokenStreamFactory: () => {
              ...
              return chain(
                [
                  ...
                  new Duplex({
                    objectMode: true,
                    write(chunk: JsonToken | JsonPackedEntryToken, _encoding, callback) {
                      if (
...
describe('::objectSieve', () => {
  ...
  it('passes root non-object primitives and arrays through the sieve without incident', async () => {
    ...
    await expect(
      feedTokenStream(
        tokenizeObject([1, 'string', true, false, null, [{ a: 1 }, { a: 1 }], { a: 1 }], {
          excludeFirstAndLast: true
        }),
        objectSieve({
          filter: [[/^.*$/, () => false]]
